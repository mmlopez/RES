...
var ball_color = {
  ...
  // Line
  link_line_width = 0.8,
  dis_limit = (window.innerHeight + 650) / 6.5,
  mouse_ball = {
    x: 0,
    y: 0,
    vx: 0,
...
function getRandomSpeed(pos) {
  ...
  switch (pos) {
    ...
    case "left":
      return [randomNumFrom(0, max), randomNumFrom(min, max)];
    default:
      return;
  }
}
  ...
  for (var i = 0; i < balls.length; i++) {
    for (var j = i + 1; j < balls.length; j++) {
      ...
      if (fraction < 1) {
        ...
        ctx.strokeStyle = "rgba(155,155,170," + alpha + ")";
...
function addBallIfy() {
  if (balls.length < 30) balls.push(getRandomBall());
}
